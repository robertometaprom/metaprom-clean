...
} from "@/lib/creative-director-v2";
import type { CreateCreativeProposalV2Input } from "@/lib/creative-director-v2";
...
function parseRequestBody(
  ...
  | { ok: true; input: CreateCreativeProposalV2Input }
...
export async function POST(req: Request) {
  ...
  try {
    const response = await createCreativeProposalV2(parsed.input);
    ...
    console.error("Creative Director V2 route error:", error);
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Creative Director V2 request failed.";
